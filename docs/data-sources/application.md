...
- `service_principal_lock_configuration` (Attributes) Specifies whether sensitive properties of a multi-tenant application should be locked for editing after the application is provisioned in a tenant. Nullable. null by default. (see [below for nested schema](#nestedatt--service_principal_lock_configuration))
